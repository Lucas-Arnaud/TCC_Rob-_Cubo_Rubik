...
# Configurar a conexão serial para os sensores (Arduino UNO na porta serial COM 3)
try:
    porta_serial_SENSORES = serial.Serial("COM3", 9600, timeout=1)
    print("Conectado à porta", porta_serial_SENSORES.portstr)
except serial.SerialException:
    print("Porta USB não detectada")
    exit()

# Configurar a conexão serial para os motores (Arduino MEGA com a RAMPS 1.4 na porta serial COM 4)
try:
    porta_serial_MOTORES = serial.Serial("COM4", 250000)
    print("Conectado à porta", porta_serial_MOTORES.portstr)
except serial.SerialException:
    print("Porta USB não detectada")
    exit()
...
comandos = {
    "D":  ['G91','G0 X0.625 F200'],
    "D'": ['G91','G0 X-0.625 F200'],
    "D2": ['G91','G0 X1.250 F200'],
    "F":  ['G91','G0 Y-0.625 F250'],
    "F'": ['G91','G0 Y0.625 F250'],
    "F2": ['G91','G0 Y1.250 F250'],
    "R":  ['G91','G0 Z-0.125 F50'],
    "R'": ['G91','G0 Z0.125 F50'],
    "R2": ['G91','G0 Z0.250 F50'],
    "L":  ['M302 P1', 'T0', 'G91','G0 E0.1 F50'],
    "L'": ['M302 P1', 'T0', 'G91','G0 E-0.1 F50'],
    "L2": ['M302 P1', 'T0', 'G91','G0 E-0.2 F50'],
    "B":  ['M302 P1', 'T1', 'G91','G0 E0.1 F50'],
    "B'": ['M302 P1', 'T1', 'G91','G0 E-0.1 F50'],
    "B2": ['M302 P1', 'T1', 'G91','G0 E-0.2 F50'],
    "U":  ['M302 P1', 'T2', 'G91','G0 E0.400 F200'],
    "U'": ['M302 P1', 'T2', 'G91','G0 E-0.400 F200'],
    "U2": ['M302 P1', 'T2', 'G91','G0 E-0.800 F200'],
}
...
def movimentar_motores(entrada):
    sequencia_movimentos = entrada.strip().upper()
    if sequencia_movimentos:
        movimentos = sequencia_movimentos.split()
        enviar_comando_motores('M302 P1')
        enviar_comando_motores('M203 E5')
        enviar_comando_motores('G91')
        for movimento in movimentos:
            if movimento in comandos:
                gcode = comandos[movimento]
                for cmd in gcode:
                    #print(f'Comando: "{movimento}": {cmd}')
                    enviar_comando_motores(cmd)
            else:
                print(f"Movimento '{movimento}' não reconhecido.")
    else:
        print("Sequência de movimentos vazia.")
...
def ler_linha_serial_sensores():
    return porta_serial_SENSORES.readline().decode('utf-8').strip()
...
def leitura_automatica():
    ...
    for comando in comandos_leitura: #Envio de cada comando de acordo com a sua categoria (sensores ou motores)
        if comando.startswith("L1") or comando.startswith("L2"):
            enviar_comando_sensores(comando)
            while True:
                resposta = ler_linha_serial_sensores()
                if resposta:
                    print(resposta)
                else:
                    break    
        else:
            movimentar_motores(comando)

    print("Leitura do cubo finalizada.")
...
# Função para aplicar movimentos ao cubo
def aplicar_movimento(cubo, movimento):
    # Separando as faces do cubo
    U = list(cubo[0:9])
    R = list(cubo[9:18])
    F = list(cubo[18:27])
    D = list(cubo[27:36])
...
def estado_para_leitura(estado):
    ...
    # Define a ordem de leitura que será lida
    ordem_leitura = [
...
def calcular_percentual_acerto(sequencia_lida, sequencia_correta):
    ...
    if len(sequencia_lida) != 48 or len(sequencia_correta) != 48:
        raise ValueError("Ambas as sequências devem ter exatamente 48 caracteres.")
    
    acertos = 0
    for i in range(48): # Verifica se cada caractere a igualdade e incrementa 1 no número de acertos
        if sequencia_lida[i] == sequencia_correta[i]:
            acertos += 1
    
    percentual_acerto = (acertos / 48) * 100 # Calcula o percentual de acerto 
    return percentual_acerto # Retorna o valor

#################################### PARTE 2 ####################################

#### FUNÇÃO MENU PRINCIPAL QUE INTEGRA TODAS AS FUNÇÕES ANTERIORES E INTERAGE COM O USUÁRIO - FLUXOGRAMA DETALHADO
def menu_principal():
    # Imprime o Cubo inicial resolvido
    ...
    # Define o cubo virtual que será modificado
    ...
    print("Cubo embaralhado (54):      ", formatar_cubo(cubo))
    ...
    for movimento in solution.split(): # Aplica os movimentos da solução no cubo virtual que está embaralhado
        cubo = aplicar_movimento(cubo, movimento)
    print("Cubo restaurado (54):       ", formatar_cubo(cubo)) # Imprime o cubo restaurado
